    ...
    @if (isset($data['recent_payroll_pay']) && $data['recent_payroll_pay']->count() > 0)
        ...
        @foreach($data['recent_payroll_pay'] as $salaryPay)
            <tr>
                <td>{{ $i }}</td>
                <td>
                    <a class="blue" href="{{ route('account.salary.payment.view', ['id' => encrypt($salaryPay->staff_id)]) }}">
                        {{ $salaryPay->reg_no }}
                    </a>
                </td>
        ...
        <tr>
            <td class="center" colspan="5">
                <a class="btn btn-success btn-minier" href="{{ route('account.payroll.balance') }}">More</a>
            </td>
        </tr>
    @else
        <tr>

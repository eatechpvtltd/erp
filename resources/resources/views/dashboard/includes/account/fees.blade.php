        ...
        @if (isset($data['recent_fees_collection']) && $data['recent_fees_collection']->count() > 0)
            ...
            @foreach($data['recent_fees_collection'] as $fee_collection)
                <tr>
                    <td>{{ $i }}</td>
                    <td>
                        <a class="blue" href="{{ route('account.fees.collection.view', ['id' => encrypt($fee_collection->students_id)]) }}">
            ...
            <tr>
                <td class="center" colspan="5">
                    <a class="btn btn-success btn-minier" href="{{ route('account.fees') }}">More</a>
                </td>
            </tr>
        @else
            <tr>

...
{{--                                {{\Carbon\Carbon::parse($application->end_date)->format('Y-m-d')}}--}}
                            ...
                            <td>
                                @if ($application->file)
                                    <a href="{{ asset('application'.DIRECTORY_SEPARATOR.$application->file) }}" target="_blank">
                                        <i class="ace-icon fa fa-download bigger-120"></i> &nbsp;{{ $application->title }} Attached File
                                    </a>
                                @else
                                    <p>No File.</p>
                                @endif
...
{{--                                        ---}}
{{--                                        <button data-toggle="dropdown" class="btn btn-primary btn-minier dropdown-toggle {{ $application->approve_status == 1?"btn-success":"btn-warning" }}" >--}}
...
{{--                                    @endif--}}
                            ...
                            <td>
                                <div class="action-buttons">
                                    ...
                                    <a href="{{ route('application.view', ['id' => encrypt($application->id)]) }}" class="btn btn-primary btn-minier">
                                        <i class="ace-icon fa fa-eye bigger-130" title="View"></i>
                                    </a>
                                    @endability

                                    @ability('super-admin','application-edit')
                                    <a href="{{ route('application.edit', ['id' => encrypt($application->id)]) }}" class="btn btn-success btn-minier">
                                        <i class="ace-icon fa fa-pencil-square-o bigger-130" title="Edit"></i>
                                    </a>
                                    @endability

                                    @ability('super-admin','application-delete')
                                    <a href="{{ route('application.delete', ['id' => encrypt($application->id)]) }}" class="btn btn-danger btn-minier bootbox-confirm" >

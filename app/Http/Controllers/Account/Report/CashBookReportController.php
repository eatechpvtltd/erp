    ...
    public function cashBook(Request $request)
    {
        ...
        if($request->all()){
            if($request->report_type && $request->start_date && $request->end_date) {
                if($request->report_type == 'daily') {
                    ...
                    foreach ($period as $key => $date) {
                        ...
                        $data[$key]['table_head'] = Carbon::parse($date)->format('d M Y');
                ...
                elseif($request->report_type == 'weekly'){
                    ...
                    foreach ($period as $key => $date) {
                        ...
                        $data[$key]['table_head'] = Carbon::parse($date)->format('d M Y') . ' - ' . Carbon::parse($date->clone()->addWeek()->subDay(1))->format('d M Y');
                        $data[$key]['total']['opening'] = $this->openingBalance($date);
                ...
                elseif($request->report_type == 'monthly'){
                    ...
                    foreach ($period as $key => $date) {
                        ...
                        $data[$key]['table_head'] = Carbon::parse($date)->format('d M Y') . ' - ' . Carbon::parse($date->clone()->addMonth()->subDay(1))->format('d M Y') ;
                        $data[$key]['total']['opening'] = $this->openingBalance($date);
                ...
                elseif($request->report_type == 'yearly'){
                    ...
                    foreach ($period as $key => $date) {
                        ...
                        $data[$key]['table_head'] = Carbon::parse($date)->format('d M Y') . ' - ' . Carbon::parse($date->clone()->addYear()->subDay(1))->format('d M Y');
            ...
            elseif (!$request->report_type && $request->start_date && $request->end_date) {
                $data['print_head'] = $this->panel.' - [ ' . Carbon::parse($request->start_date)->format('d M Y') . ' - ' . Carbon::parse($request->end_date)->format('d M Y') . ']';
            ...
            $data['print_head'] = $this->panel.' - '.Carbon::parse($date)->format('d M Y');

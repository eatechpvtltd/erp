...
class OnlinePaymentController extends CollegeBaseController
{
   /* protected $base_route = 'info.smsemail';
    ...
    public function paymentView(Request $request, $id)
    {
        ...
        $refText  = json_decode($data['student'][0]->ref_text);
        $textDetail=[] ;
            foreach($refText as $keyfield => $text) {
                $textDetail[$keyfield] = $text;
                if($keyfield == 'pidx'){
                    $data['khaltiStatus']  = $this->KhaltiPaymentStatus($text);
    ...
    public function paymentVerify(Request $request, $id)
    {
        ...
        if($request->verifyMethod =='manually'){
            //update online payment table with verified
                $onlinePaymentDetail->update(
                    [
                        'verify_method' =>   $request->verifyMethod,
                        'status'=>'active',
                    ]
                );
            $request->session()->flash($this->message_success, 'Mark As Manually Verify');
        }else{
    ...
    public function KhaltiPaymentStatus($pidx)
    {
        $curl = curl_init();
        curl_setopt_array($curl, array(
            ...
            CURLOPT_HTTPHEADER => array(
                'Authorization: Key 0b03fa751f7245b68305a81e37b093cc',
        ...
        //return $responseValues = json_decode($response);

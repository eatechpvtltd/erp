...
class StudentStatusController extends CollegeBaseController
{
    ...
    protected $view_path = 'academic.status.student-status';
    protected $panel;
    protected $filter_query = [];

    public function __construct()
    {
        $this->panel = __('academic.child.status_setting.child.student_status');
    }
            ...
            if ($request->has('chkIds')) {
                foreach ($request->get('chkIds') as $row_id) {
                    switch ($request->get('bulk_action')) {
                        case 'active':
                        case 'in-active':
                            $row = StudentStatus::find(decrypt($row_id));
                            if ($row) {

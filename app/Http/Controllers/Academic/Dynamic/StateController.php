...
class StateController extends CollegeBaseController
{
    protected $base_route = 'dynamic.state';
    protected $view_path = 'academic.dynamic-hub.state';
    protected $panel;
    protected $filter_query = [];

    public function __construct()
    {
        $this->panel = __('academic.child.dynamic_gallery.child.state');
    }
    ...
    public function edit(Request $request, $id)
    {
        $id = decrypt($id);
        $data = [];
        if (!$data['row'] = State::find($id))
            return parent::invalidRequest();

        $data['state'] = State::orderBy('title')->get();
    ...
    {
        $id = decrypt($id);
       if (!$row = State::find($id)) return parent::invalidRequest();

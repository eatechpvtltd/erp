...
class AcademicLevelController extends CollegeBaseController
{
    protected $base_route = 'dynamic.academic-info-level';
    protected $view_path = 'academic.dynamic-hub.academic-info-level';
    protected $panel;
    protected $filter_query = [];

    public function __construct()
    {
        $this->panel = __('academic.child.dynamic_gallery.child.academic_info_level');
    }

    public function index(Request $request)
    {
        $data = [];
        $data['academic-info-level'] = AcademicInfoLevel::get();
    ...
    public function store(AddValidation $request)
    {
       ...
       AcademicInfoLevel::create($request->all());
    ...
    public function edit(Request $request, $id)
    {
        $id = decrypt($id);
        $data = [];
        if (!$data['row'] = AcademicInfoLevel::find($id))
            return parent::invalidRequest();

        $data['academic-info-level'] = AcademicInfoLevel::orderBy('title')->get();
    ...
    {
        $id = decrypt($id);
       if (!$row = AcademicInfoLevel::find($id)) return parent::invalidRequest();
            ...
            if ($request->has('chkIds')) {
                foreach ($request->get('chkIds') as $row_id) {
                    switch ($request->get('bulk_action')) {
                        case 'active':
                        case 'in-active':
                            $row = AcademicInfoLevel::find($row_id);
                            if ($row) {

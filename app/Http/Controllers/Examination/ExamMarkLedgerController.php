...
class ExamMarkLedgerController extends CollegeBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        if($year && $month && $exam && $faculty && $semester && $subject) {
            ...
            $data['ledger_exist'] = ExamMarkLedger::select('exam_mark_ledgers.exam_schedule_id', 'exam_mark_ledgers.students_id',
                ...
                'exam_mark_ledgers.status', 's.id as student_id', 's.batch','s.reg_no', 's.first_name', 's.middle_name', 's.last_name',
                's.last_name')
    ...
    public function studentHtmlRow(Request $request)
    {
        ...
        /*For Student List*/
        $studentCondition = [['faculty', '=' , $faculty], ['semester', '=' , $semester] , ['batch', '=' , $batch] ];
        ...
        if($examScheduleId){
            ...
            $ledgerExist = ExamMarkLedger::select('exam_mark_ledgers.exam_schedule_id',
                ...
                's.id as student_id','s.reg_no','s.university_reg','s.first_name','s.middle_name','s.last_name','s.batch')
                ->where('exam_mark_ledgers.exam_schedule_id',$examScheduleId)
                ->where('s.batch', intval($batch))
                ->join('students as s','s.id','=','exam_mark_ledgers.students_id')
                ->orderByRaw('s.reg_no*1 asc')
                ->get();
            ...
            $activeStudent = Student::select('id','reg_no','university_reg','first_name','middle_name','last_name')
                ->where($studentCondition)
                ->whereNotIn('id',$existStudentId)
                ->Active()
                //->orderBy('id','asc')
               //->orderBy('reg_no','asc')
                ->orderByRaw('reg_no*1 asc')
                //->orderBy('university_reg','asc')
                //->orderByRaw('reg_no*1 asc')
                ->get();

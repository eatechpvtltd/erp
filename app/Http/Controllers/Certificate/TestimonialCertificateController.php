...
use App\Http\Requests\Certificate\Testimonial\AddValidation;
use App\Http\Requests\Certificate\Testimonial\EditValidation;
use App\Models\CertificateTemplate;
use App\Models\CharacterCertificate;
use App\Models\CertificateHistory;
use App\Models\ProvisionalCertificate;
use App\Models\Student;
use App\Models\TestimonialCertificate;
use Carbon\Carbon;
use Illuminate\Http\Request;
use URL;
use ViewHelper;
class TestimonialCertificateController extends CollegeBaseController
{
    protected $base_route = 'certificate.testimonial';
    protected $view_path = 'certificate.testimonial';
    protected $panel = 'Testimonial Certificate';
    protected $folder_path;
    protected $codePrefix = "TMC";
        ...
        $data['student'] = Student::select('students.id','students.reg_no', 'students.reg_date',
            ...
            ->where(function ($query) use ($request) {
                ...
                if ($request->has('issue-start-date') && $request->has('issue-end-date')) {
                    $query->whereBetween('tmc.date_of_issue', [$request->get('issue-start-date'), $request->get('issue-end-date')]);
                    ...
                    $query->where('tmc.date_of_issue', '>=', $request->get('issue-start-date'));
                    $this->filter_query['issue-start-date'] = $request->get('issue-start-date');
                } elseif ($request->has('issue-end-date')) {
                    $query->where('tmc.date_of_issue', '<=', $request->get('issue-end-date'));
                    $this->filter_query['issue-end-date'] = $request->get('issue-end-date');
                }

                if ($request->has('year')) {
                    $query->where('tmc.year', '=', $request->year);
                    $this->filter_query['tmc.year'] = $request->year;
                }

                if ($request->has('tmc_num')) {
                    $query->where('tmc.tmc_num', 'like', '%'. $request->tmc_num.'%');
                    $this->filter_query['tmc.tmc_num'] = $request->tmc_num;
                }

                if ($request->has('character')) {
                    $query->where('tmc.character', 'like', '%'. $request->character.'%');
                    $this->filter_query['tmc.character'] = $request->character;
                }
    ...
    public function add(Request $request, $id)
    {
        ...
        $certificateExist = TestimonialCertificate::where('students_id',$id)->first();
        if($certificateExist){
    ...
    public function store(AddValidation $request)
    {
        ...
        $data['TranscriptData'] = $TranscriptData = $this->getTranscriptData($id);
        $request->request->add(['program_duration' => isset($data['TranscriptData']->duration)?$data['TranscriptData']->duration:""]);
        ...
        $request->request->add(['average_grade' => isset($data['TranscriptData']->transcriptGL)?$data['TranscriptData']->transcriptGL:""]);
        ...
        $row = TestimonialCertificate::create($request->all());
    ...
    public function update(EditValidation $request, $id)
    {
        ...
        $updateData = [
            "tmc_num"               => $request->tmc_num,
            "ref_num"               => $request->ref_num,
            "date_of_issue"         => Carbon::parse($request->date_of_issue),
            "year"                  => $request->year,
            "scale"                 => isset($data['TranscriptData']->scale)?$data['TranscriptData']->scale:"",
            "program_duration"      => isset($data['TranscriptData']->duration)?$data['TranscriptData']->duration:"",
            "gpa"                   => isset($data['TranscriptData']->transcriptGPA)?$data['TranscriptData']->transcriptGPA:"",
            "average_grade"         => isset($data['TranscriptData']->transcriptGL)?$data['TranscriptData']->transcriptGL:"",
            "ref_text"              => json_encode($request->except('_token')),
            "last_updated_by"       => auth()->user()->id,
        ];


        $row = $student->testimonialCertificate()->update($updateData);
        if($row){
            //manage history
            $CreateHistory = CertificateHistory::create([
                'students_id' => $student->id,
                'certificate' => 'testimonial',
                'certificate_id' => $student->testimonialCertificate()->first()->id,
    ...
    public function delete(Request $request, $id)
    {
        ...
        $certificate = $row->testimonialCertificate()->first();
        //delete history
        ...
        if ($request->has('bulk_action') && in_array($request->get('bulk_action'), ['print','active', 'in-active', 'delete'])) {
            ...
            if ($request->has('chkIds')) {
                foreach ($request->get('chkIds') as $row_id) {
                    ...
                    $row = TestimonialCertificate::find($row_id);

                    switch ($request->get('bulk_action')) {
                        case 'print':
                            $data['certificate_template'] = $certificateTemplate = CertificateTemplate::where('certificate','=','TESTIMONIAL')
                                ->first();
                            $data['student'] = $this->bulkPrint($certificateTemplate, $request->get('stuIds'));
                            return view(parent::loadDataToView('print.certificate.testimonial'), compact('data'));
                            break;
                        case 'active':
                        case 'in-active':
                            if ($row) {
    ...
    public function bulkPrint($certificateTemplate, $studIds)
    {
        ...
        $filteredStudent = $students->filter(function ($student, $key) use($certificateTemplate) {
            $data = Student::select('students.id','students.reg_no', 'students.reg_date', 'students.university_reg',
                ...
                'tmc.id as certificate_id','tmc.date_of_issue', 'tmc.ref_num', 'tmc.tmc_num',
                'tmc.program_duration', 'tmc.year', 'tmc.gpa', 'tmc.scale', 'tmc.average_grade')

    ...
    public function bulkPrint($certificateTemplate,$studIds)
    {
        ...
        $filteredStudent = $students->filter(function ($student, $key) use($certificateTemplate) {
            $data = Student::select('students.id','students.reg_no', 'students.reg_date', 'students.university_reg',
                ...
                'tc.id as certificate_id','tc.date_of_issue', 'tc.date_of_leaving', 'tc.tc_num',
                'tc.join_time_class','tc.leaving_time_class', 'tc.qualified_to_promote','tc.paid_fee_status','character',
                'tc.fee_concession_detail', 'tc.exam_fail_status', 'tc.subject_studies', 'tc.last_taken_exam_with_result', 'cadet_detail',
                'tc.reason_to_leave', 'tc.total_attendance', 'tc.school_college_open_total','tc.any_other_remark','birth_place', 'promoted_class',
                'tc.application_date', 'tc.dob_certificate', 'tc.school_category', 'tc.extra_activity_detail',
                'tc.ref_text',)
